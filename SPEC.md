...
- `/quotes`: `CollectionPage` whose `hasPart` is one `Quotation` per line — text, creator Person JP Ahonen, dateCreated, and `isPartOf` the Article or DefinedTerm it comes from. The same nodes are injected as `hasPart` on each essay's `Article`, so a line stays attributable when only the essay is fetched. `isPartOf` is emitted only for lines verified as a literal substring of the cited work; generated by `tools/quotes/generate.py` from `content/quotes.md`.
